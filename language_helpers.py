...
import numpy as np
import collections
from parameters import BATCH_SIZE, FLAGS
...
def load_dataset(max_length, max_n_examples, tokenize=False, max_vocab_size=2048, data_dir=FLAGS.DATA_DIR, pad=True, dataset='training'):
    
    assert dataset == 'training' or dataset == 'heldout', "only available datasets are 'training' and 'heldout'"
    lines = []

    finished = False
    number_of_divided_files = 64 if dataset == 'training' else 16

    for i in range(number_of_divided_files-1):
        if dataset == "heldout":
            path = data_dir +  ( "/{}/data_{}_of_80".format(dataset,str(i + 65)) )
        else:
            path = data_dir +  ( "/{}/data_{}_of_80".format(dataset,str(i + 1)) )
