...
from define_objective_function import get_optimization_ops, define_objective
...
def run(iterations, seq_length, is_first, charmap, inv_charmap, prev_seq_length):
    ...
    lines, _, _ = helper_methods.load_dataset(seq_length=seq_length, b_charmap=False, b_inv_charmap=False, n_examples=MAX_N_EXAMPLES)

    real_inputs_discrete = tf.placeholder(tf.int32, shape=[BATCH_SIZE, seq_length])

    global_step = tf.Variable(0, trainable=False)
    disc_cost, gen_cost, fake_inputs, disc_fake, disc_real, disc_on_inference, inference_op, other_ops = define_objective(charmap,real_inputs_discrete, seq_length, rnn_cell=rnn_cell)
    ...
    with tf.Session(config=config) as session:
        ...
        for iteration in range(iterations):
            ...
            if iteration % FLAGS.PRINT_ITERATION == FLAGS.PRINT_ITERATION-1:
                _data = next(gen)
                summary_str = session.run( merged, feed_dict={real_inputs_discrete: _data} )

                tf.logging.warn("iteration %s/%s"%(iteration, iterations))
                tf.logging.warn("disc cost {} gen cost {} average step time {}".format( np.mean(_disc_cost_list), np.mean(_gen_cost_list), np.mean(_step_time_list)) )
                ...
                log_samples(decode_indices_to_string(_data, inv_charmap), real_scores, iteration, seq_length, "gt")
